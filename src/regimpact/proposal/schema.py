...
class RuleState:
    ...
    max_ltv: Optional[float] = None          # 무주택 일반(STANDARD) 세그먼트 값
    ltv_by_segment: dict = field(default_factory=dict)   # 세그먼트 → LTV
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "ltv_by_segment": dict(self.ltv_by_segment),
...
class RuleChangeProposal:
    ...
    summary: str = ""
    # 같은 세그먼트에 서로 다른 값이 추출된 경우. 조용히 하나를 고르지 않고 남긴다.
    conflicts: list[str] = field(default_factory=list)
    # LTV 카테고리인데 수치를 파싱할 수 없던 항목(전입의무·만기제한 등). 버리지 않는다.
