...
RegImpact AI는 새 문서 하나를 요약하는 시스템이 아니다. 핵심 질문은
...
class PolicyVersion:
    ...
    def is_pending_at(self, as_of: date) -> bool:
        """확정됐지만 아직 시행 전 — '다음 정책'.

        이전 판은 `is_pending` 프로퍼티였는데 `status is CONFIRMED` 만 봤다. 시행일 비교가
        없으니 이미 시행 중인 정책도 전부 '시행 예정'이 됐다. 시점 없이는 판정할 수 없는
        질문이라 메서드로 바꿨다.
        """
        return self.status is PolicyStatus.CONFIRMED and as_of < self.effective_from
