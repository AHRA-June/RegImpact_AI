...
from ..regions import normalize_region_name, region_label
from .version import (
    PolicyStatus,
    PolicyVersion,
    Provenance,
...
def _map_region(name_or_code: str) -> Optional[str]:
    """추출된 지역 표기를 코드로 매핑. 확신이 없으면 None(넘겨짚지 않는다).

    새로 지정되는 지역은 기준선에 아직 없는 것이 정상이므로(그게 이 정책이 하는 일이다),
    `REGION_VERSIONS` 등록 여부는 조건이 아니다. 별칭표로 코드가 나오는지만 본다.
    """
    return normalize_region_name(name_or_code)
