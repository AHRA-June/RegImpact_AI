...
지역 해석은 `regions.REGION_VERSIONS`(지금까지 확정된 사실의 누적 기준선) 위에
...
from ..regions import canonical_code, region_label, resolve_region_status
...
def preview_region_impact(policy: PolicyVersion) -> list[RegionChange]:
    ...
    `already_in_baseline=True` 인 항목은 기준선(regions.REGION_VERSIONS)이 이미 같은 결론을 내고 있다는 뜻이다
    ...
    for d in policy.region_deltas:
        ...
        # before = 시행 **전날**의 상태. 이전 판은 두 값을 같은 시점으로 조회해서
        # (복붙 실수) before 가 이미 지정된 뒤 상태를 가리켰다 — 미리보기가
        # "REGULATED → REGULATED" 로 보여 변화가 사라졌다.
        before, _ = resolve_region_status(code, d.effective_from - timedelta(days=1))
        baseline_after, _ = resolve_region_status(code, d.effective_from)
        out.append(RegionChange(
            region_code=code,
            region_name=d.region_name or region_label(code),
            before=before,
