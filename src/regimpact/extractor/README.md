...
- **백엔드 교체 가능:** Gemini(Google AI Studio)는 **SDK 없이 urllib REST**로 동작(추가 의존성 0).
  Anthropic은 SDK(`pip install ".[llm]"`). 둘 다 structured output(JSON Schema)로 스키마 강제.
...
- `extractor.py` — `extract_regchange()` + `gemini_completion()`(gemini-2.5-flash, REST) /
  `anthropic_completion()`(claude-opus-5, SDK) + `to_gemini_schema()`(JSON Schema→Gemini 방언 변환)
...
```
백엔드/모델 강제: `REGIMPACT_LLM=gemini|anthropic`, `REGIMPACT_MODEL=...`.
