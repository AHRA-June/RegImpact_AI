...
6. **예외는 하나로 뭉치지 말고 개별 항목으로 분리한다.** 여러 예외가 한 문장에 나열돼도
   ("생애최초·서민실수요자·정책모기지 등") 각 예외를 **별도 EXCEPTION change 항목**으로 만들고,
   summary에 그 예외명을 명시하라(예: "서민·실수요자 완화 LTV 적용"). 뭉뚱그린 요약은 개별
   예외를 놓치게 한다. 각 항목은 여전히 자기 근거를 원문에서 verbatim 인용해야 한다.

카테고리:
- LTV: 담보인정비율 변경
- EXCEPTION: 예외(완화 LTV/적용제외 등). 규제지역에서 완화 LTV가 적용되는 대상은 각각 별도 추출.
  최소한 다음을 개별 확인: ① 생애최초 ② 서민·실수요자 ③ 정책모기지(디딤돌·보금자리 등).
  원문에 없는 예외는 지어내지 말 것(규칙 1 우선).
- GRANDFATHERING: 경과규정(종전규정 적용 조건)
- EFFECTIVE_DATE: 시행일
- REGION: 규제지역 신규 지정
- SCOPE_LIMIT: 전세대출·신용대출·중도금·사업자대출 제한(자동판정 밖)

목표는 사람이 검토할 초안이다. 정확한 인용과 낮은 환각이 완전성보다 우선이되, **원문에 명시된
예외를 개별로 빠짐없이** 담는 것이 이 태스크의 핵심 완전성 목표다."""
