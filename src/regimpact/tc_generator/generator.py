...
def conflict_cases() -> list[GeneratedCase]:
    ...
    return [
        ...
        _case("CFL-04", Category.CONFLICT,
              "유주택(비처분 1주택) + 생애최초 → 입력 모순 → NEEDS_HUMAN_REVIEW (Q8 확정)",
              _app(house_count=1, disposal_condition_flag=False, first_home_buyer=True)),
        _case("CFL-09", Category.CONFLICT,
              "다주택 + 생애최초 → 입력 모순. 다주택 0%보다 무결성 게이트가 앞선다",
              _app(house_count=2, first_home_buyer=True)),
        _case("CFL-10", Category.CONFLICT,
              "유주택 + 생애최초 + 경과규정 → 모순 게이트가 경과규정(P1)보다 앞",
              _app(house_count=1, first_home_buyer=True,
                   application_accepted_at=_CUTOFF)),
        _case("CFL-11", Category.CONFLICT,
              "정책대출 + 유주택 + 생애최초 → P0b가 먼저, Discovery로 분리 (모순 판정 안 함)",
              _app(house_count=1, first_home_buyer=True, policy_mortgage_flag=True)),
        _case("CFL-13", Category.CONFLICT,
              "처분조건부 플래그 + 보유 0채 → 처분할 주택이 없음 → 입력 모순 (Q12)",
              _app(house_count=0, disposal_condition_flag=True)),
        _case("CFL-14", Category.CONFLICT,
              "처분조건부 플래그 + 다주택 2 → 필드가 전제한 '1주택'이 아님 → 입력 모순 (Q12)",
              _app(house_count=2, disposal_condition_flag=True)),
        _case("CFL-15", Category.CONFLICT,
              "house_count 음수 → 무효값 (Q12)",
              _app(house_count=-1)),
        _case("CFL-16", Category.CONFLICT,
              "계약금 납부일만 있고 계약체결일 없음 → 모순 아님(가계약금 실재). G2 미성립 40%",
              _app(house_count=0, downpayment_paid_at=_CUTOFF)),
        _case("CFL-12", Category.CONFLICT,
              "처분조건부 1주택 + 생애최초 → 모순 아님(§E-138), 유효 조합 70%",
              _app(house_count=1, disposal_condition_flag=True, first_home_buyer=True)),
