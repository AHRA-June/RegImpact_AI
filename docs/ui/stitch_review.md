...
> ## ✅ 해결됨 (2026-08-18) — 정정 프롬프트 대신 **코드 생성**으로 갔다
>
> 아래의 "Stitch에 정정 프롬프트를 다시 넣어 재생성" 방식은 **채택하지 않았다.** 재생성해도
> 다음 데이터 변경 때 또 환각하고, 사람이 매번 눈으로 잡아야 하기 때문이다.
> 대신 **디자인만 보존하고 화면을 코드에서 생성**한다 → `src/regimpact/ui/`, 산출물 `docs/ui/generated/`.
>
> - 값 출처: 추출기·룰엔진·TC 회귀의 실제 출력 (리터럴 도메인 수치 금지)
> - 디자인: `stitch_export`의 토큰을 verbatim 사용 + 정적 CSS로 자기완결(CDN 의존 제거)
> - 아래 표의 ❌·⚠️ 항목은 전부 `tests/test_ui.py`의 **회귀 테스트로 고정**됐다
>   (환각 지역명 / `60%→50%` / 경과규정 부등호 / 값 하드코딩 → 재발 시 테스트 실패).
>
> 이 문서는 **사고 기록**으로 보존한다. 아래 내용은 2026-08-10 시점 그대로다.
