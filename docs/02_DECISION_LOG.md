...
### 2026-08-18 (6) · Policy Version DB + Temporal Policy Resolver · ✅ 채택
- **요청:** "새 정책이 나왔을 때 업로드하는 기능 + 현재/다음 정책을 날짜로 버전 관리."
  브리프 §7(Temporal Policy Resolver)·§19(Policy Version DB 등록)가 이미 스키마를 확정해 둔 항목이며,
  §18 "자르면 안 되는 것" 목록에 들어 있다.
- **저장소는 git 안의 JSON 파일**(`docs/policies/*.json`). DB 서버가 아닌 이유: 정책 버전은 감사 대상이고,
  git 이력이 곧 "누가 언제 무엇을 확정했는가"의 기록이 된다(LOCKED §10).
- **정책은 문서가 아니라 시행일 구간을 갖는 버전.** 브리프 §7의 메타데이터 스키마를 그대로 구현
  (policy_id·source_document_id·title·issuer·published_at·effective_from·effective_to·
  supersedes_policy_id·jurisdiction·policy_scope·version·source_url·source_hash·retrieved_at).
- **DRAFT 는 판정에 쓰이지 않는다(LOCKED §4).** 업로드·LLM 추출 결과는 항상 `DRAFT`+`AI_DRAFT` 이며,
  사람이 `confirm()` 해야 `CONFIRMED`+`HUMAN_CONFIRMED` 가 되어 규제상태 해석에 반영된다. 테스트가 강제.
- **매핑 못 한 지역명은 조용히 버리지 않는다.** 추출된 지역 표기를 코드로 확신 있게 매핑하지 못하면
  `unmapped_regions` 로 보고하고 비워 둔다 — 넘겨짚는 순간 근거 없는 규제상태가 만들어진다.
  (강남 70% 사고와 같은 부류의 실패를 업로드 경로에서 미리 막는 장치.)
- **오버레이 방식(전역 상태 변경 없음).** 지역 기준선(`regions.REGISTRY`, 지금까지 확정된 사실의 누적)을
  그대로 두고, 정책 delta 를 순수 함수로 얹어 해석한다(`resolve_region_with_policies`).
  delta 가 없으면 기준선과 **완전히 동일** — 기존 판정을 바꾸지 않는다(테스트로 고정).
  덕분에 업로드한 정책을 "적용해보기"만 하고 되돌리는 것이 자유롭다.
- **시드 4건 등록:** MOLIT_20161103(강남4구 조정) → MOLIT_20170803(투기과열) → MOLIT_20251016(서울21+경기12)
  → FSC_20260630(경기3). 전부 MOLIT 참고2 현황표에서 유도. **정책 delta 합집합이 지역 기준선의 규제지역
  집합과 일치하는지**를 테스트가 검사한다(두 구조 간 드리프트 탐지).
- **업로드 UI(Streamlit "정책 버전" 탭):** 원문 업로드 → SHA-256 스냅샷 → 메타·지역 입력 → 초안 →
  적용 미리보기(before/after) + 중첩 경고 → 사람 확정 체크 → 세션 등록 + JSON 다운로드.
  Streamlit Cloud 파일시스템은 세션 종료 시 사라지므로 **"저장소에 커밋해야 영구 반영"을 화면에 명시**한다.
- **E2E 반영:** 파이프라인 2단계(Policy Version Resolution)가 이제 Policy Version DB 를 읽어
  현재/직전/예정 정책을 보고하고, 검증보고서에 **정책 버전 타임라인** 섹션이 추가됐다.
- **검증:** `tests/test_policy.py` 35건 (총 146). 원문 지역 수 대조 · DRAFT 무효 · 오버레이 no-op ·
  미매핑 보고 · 중첩 경고 · 타임라인 상태.

### 2026-08-18 (5) · Q12 해소 — 입력 모순 3건 추가, 가계약금 관련 2건 보류 · ✅ 채택
- **채택:** 처분조건부 플래그인데 보유 0채 / 처분조건부인데 다주택 / house_count 음수.
  앞의 둘은 `§A` 스키마가 "처분조건부 1주택은 house_count==1과 함께"로 정의하므로 근거가 직접 있다.
- **미채택:** 계약금 납부일만 있고 계약체결일 없음 / 계약금이 계약보다 빠름.
  **🔺 이전 권고 정정** — Q12 제기 시 앞의 것을 "넣는 쪽이 정직"이라 적었으나, **가계약금**(정식 계약 전
  계약금 선납)이 실무에 실재하므로 두 후보는 같은 질문이다. 기계적으로 막으면 정상 건을 사람 검토로 보낸다.
  게다가 §F G2 가 계약+계약금을 쌍으로 요구하므로 두 경우 모두 **경과규정이 성립하지 않을 뿐**이고,
  그건 이미 올바른 동작이다.
- 회귀 `CFL-13·14·15`(모순) + `CFL-16`(가계약금은 모순 아님을 고정) → 60 케이스 100%.

### 2026-08-18 (4) · Q8 해소 — 입력 무결성 게이트(P0c) 신설 · ✅ 채택
- **먼저, 이건 상충이 아니었다.** `§E-138` 이 "처분조건부 1주택 AND 생애최초"를 **유효 조합**으로 명시하므로
  `§E-139` 의 "유주택"은 `is_owner()`(다주택 + 비처분 1주택) 의미다. 그렇다면 §E-139 는 우선순위 규칙이
  아니라 **입력 유효성 규칙**이고, §H(유효한 입력에 어떤 LTV를 주는가)와 층위가 달라 **둘 다 참**이다.
  §H 에 검증 게이트가 없었을 뿐이다. → "§E냐 §H냐" 택일 프레임을 폐기하고 게이트를 추가하는 것으로 해소.
- **결정: 0% 자동판정 → `NEEDS_HUMAN_REVIEW`.** 근거:
  ① 0%를 내주면 "정상 입력이고 답이 0%"와 "입력이 모순인데 우연히 0%"를 시스템이 구분하지 못한다.
  ② 0%는 무해한 답이 아니라 **대출 거절**이고, 틀린 쪽이 `first_home_buyer` 플래그였다면 정답은 70%다.
     0%와 70% 사이 70%p 격차가 데이터 오류 하나 뒤에 숨는 구조는 두지 않는다.
  ③ 이미 두 번 적용한 원칙과 같은 부류다 — `OWNER_BASELINE_UNKNOWN`, `UNKNOWN_REGION`.
     **모르면 지어내지 않고 사람에게 넘긴다** (LOCKED §10).
- **게이트 위치:** P0(스코프)·P0b(정책대출) 뒤, P1(경과규정) 앞. 스코프 밖이면 모순을 따질 필요가 없고,
  모순 입력에 종전규정 70%를 내주는 것도 0%만큼 위험하다.
- **과잉 차단 방지:** `is_owner()` 기준이라 처분조건부 1주택 + 생애최초(§E-138 유효 조합)는 잡지 않는다.
  회귀 `CFL-12` 가 이 경계를 고정한다.
- **오라클:** `validation.py` 를 import 하지 않고 규칙표 상단에 조건을 직접 재기입(판정 구현 독립성 유지).
- **범위 절제:** 같은 부류의 다른 모순 후보(처분조건부인데 보유 0채, 계약금만 있고 계약 없음 등)는
  **임의로 추가하지 않고** `03_OPEN_QUESTIONS.md` Q12 로 표면화했다. LOCKED §4 — 확정된 것만 넣는다.
- **검증:** 회귀 56 케이스 Pass 100%(CONFLICT 12로 확대), pytest 110, JS 포팅 56/56 일치.

### 2026-08-18 (3) · Impact Matrix E2E — 설계 결정 4건 · ✅ 채택
- **① 합성 포트폴리오는 '시장 분포'가 아니라 '커버리지 격자'다.** LOCKED §8로 실제 고객데이터를 쓸 수 없는데
  현실 비중을 지어내면 그 숫자가 근거 없는 '영향 고객 수'로 둔갑한다. 그래서 난수 없이 완전요인 등가중
  (지역군 4 × 차주 6 × 경과규정 이벤트 4 × 가격 7 = 2,016)으로 만들고, 산출물을 "몇 명이 영향받는다"가 아니라
  "어떤 세그먼트가 어떻게 바뀌는가"로 읽도록 문구까지 고정했다. 실제 분포가 생기면 `weight` 만 채우면 된다.
- **② 헤드라인 비율은 층을 나눠 읽는다.** 격자에서 경과규정 이벤트가 3/4을 차지하므로 전체 혼합 영향률(3.1%)은
  현실이 아니라 설계의 부산물이다. 세그먼트 평균 LTV도 섞으면 "70%→63%"로 왜곡된다(실제 70%→40%).
  → 헤드라인·세그먼트는 **경과규정 미해당 층**에서, 경과규정 효과는 **반사실**(이벤트 제거 후 재판정)로 분리.
- **③ 매트릭스 행은 유도하고, 근거 출처를 행마다 표시한다.** 행 내용을 프로즈로 박으면 정책이 바뀌어도 표가
  안 바뀐다. 그래서 추출·고객영향·회귀·지역 레지스트리에서 유도한다. 또 `Provenance`(결정론 엔진 / 사람 확정
  골드 / AI 추출 / 확정 명세 / 미실행)를 열로 둔다 — "AI가 만들었다"와 "사람이 확정했다"가 한 표에서
  구분되지 않으면 감사에 쓸 수 없다(LOCKED §10).
- **④ 안 돌린 단계를 감추지 않는다.** LLM 추출은 키가 없으면 `MISSING` 으로 남고, 그 여파(근거가 골드로 대체됨,
  Citation/Completeness 가 N/A)를 파이프라인·보고서·웹 모두에 그대로 노출한다. 채워넣는 순간 포트폴리오가
  아니라 마케팅이 된다.
- **Phase(시간축) 보존:** LOCKED §7. 3구간(D-day 전 / 시행 후 / 별도 트리거) 모두 존재해야 테스트가 통과한다.
- **Discovery 격리:** 전세·신용·중도금·사업자대출은 매트릭스에 표시만 하고 자동처리=수동으로 고정(브리프 §5.2·§24-11).
- **검증:** 테스트 27건 추가(총 108). 재현성·Phase 보존·Discovery 격리·미실행 은폐 금지·층 분리를 계약으로 고정.

### 2026-08-18 (2) · Q9·Q10 확정 + 경과규정 종전규정 해석 수정 · ✅ 사용자 확정
- **Q9 — 비규제 유주택 60% 확정.** 사용자 확정. 근거 MOLIT 참고1 "非규제지역(수도권 外) 무주택 70% / 유주택 60%".
  원문이 **수도권 外**를 명시하므로 비수도권에만 적용하고, 수도권 비규제 유주택은 근거 부재로 escalation 유지
  (Q9 제기 시 제시한 기본값 채택 — 수도권도 60%로 통일하려면 P4의 `capital_area` 분기 한 줄).
- **Q10 — P3(다주택)을 지역 분기 앞으로 이동.** 사용자 확정. 근거 FSC p2 C06 "다주택자는 수도권 內 주택구입시
  규제지역 여부와 무관하게 LTV 0%" + §C-1b R6. 조건 = 다주택 AND (규제지역 OR 수도권).
  비수도권 비규제 다주택은 Q9의 유주택 기준 60%로 수렴한다.
- **Q11 — 경과규정 '종전규정'을 70% 고정에서 컷오프 시점 재판정으로 수정 (버그).** Q9·Q10 반영 중 발견.
  지역이 3곳뿐일 때는 우연히 맞았으나(그 3곳은 종전에 비규제 수도권), 전국 레지스트리 도입 후
  **이미 규제지역이던 강남에 경과규정이 붙으면 70%** 라는 오답이 나왔다. 명세 §F가 "종전규정 적용"이라 했을 뿐
  70%라 한 적이 없으므로 도메인 판단 항목이 아니라 명세 준수 방향의 수정으로 처리했다.
- **LOCKED §4 절차:** 규칙 '값'(60%)과 '우선순위'(P3 이동) 변경은 **사용자 확정 후에** 반영했다.
  Q11은 값 신설이 아니라 명세 해석 오류 수정이므로 즉시 반영하고 여기 기록한다.
- **명세 개정:** `05_RULE_SPEC` §C-2(수도권/비수도권 2열 표로 확장), §F 주석, §H 의사코드 전면 개정.
  개정 전 판본은 git 이력으로 추적.
- **오라클 재작성:** 종전 오라클은 docstring이 "선언적 표 + 범용 해석기"라 주장했으나 실제로는 엔진과 같은
  명령형 분기였다. 이번에 **진짜 규칙표(`_RULE_TABLE`) + 첫 매치 해석기**로 재작성해 구조적 독립을 실현했다.
- **회귀:** 52 케이스 Pass Rate 100% (EXCEPTION 10 / GRANDFATHERING 9 / CONFLICT 8로 확대), pytest 80 통과,
  JS 포팅본 52/52 + 지역 241×7 일치.
