...
> **이전 버전:** 없음 — 최초 작성
> **버전 규칙:** `docs/features/_VERSIONING.md` (수정 시 `regchange-ai_v2.md` 신규 + 이 폴더 CHANGELOG).

## 변경 이력 (→ v1)
- 최초 작성 (신규). 브리프 §25 필수 항목을 구체화하고, **현행 구현 상태(구현됨/부분/계획)**를 명시.
  이전 버전 없음. 근거: `docs/00_BRIEF.md`, `docs/05_RULE_SPEC.md`, `docs/metrics_spec.md`, `docs/features/`.
...
| RAG / retrieval | ⬜ 계획 | 현재 원문 전량을 프롬프트에 주입(문서 3건). 검색층은 후속 |
...
- ✅ **완료:** 룰엔진, Extractor+Citation Assurance, Impact Matrix(+UI), Rule Proposal+Human Review,
  TC Generator+3계층 회귀, Assurance 4 dimension 스코어카드+임계값, Validation Report(md+HTML).
- ◐ **부분:** Temporal Policy Resolver(지역만), Minimal UI(2화면 데이터바인딩), Audit(approval만).
- ⬜ **계획:** RAG/retrieval, 정식 Audit Logger, 정식 15~20쪽 검증보고서 서사, 자동 LLM API 실행,
