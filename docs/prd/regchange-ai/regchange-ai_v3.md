...
> **버전:** v3   **날짜:** 2026-08-13   **상태:** 유효(현행 구현 반영)
> **이전 버전:** v2 (`regchange-ai_v2.md`)
> **버전 규칙:** `docs/features/_VERSIONING.md` (수정 시 `regchange-ai_v4.md` 신규 + 이 폴더 CHANGELOG).

## 변경 이력 (v2 → v3)
- **[범위]** Audit(Logger·event): `⬜ 계획/부분` → `✅ 구현`. `src/regimpact/audit/`(해시 체인 감사로그,
  변조 탐지). §5·§6·§10·§13·§15 반영. Risk Register R-GOV-01 잔여 6→3.
...
| Audit event | ✅ | `audit.AuditEvent`(seq·timestamp·actor·action·target·details·prev_hash·entry_hash) — 해시 체인 |
...
| Audit Logger | ✅ | `audit/` — append-only 해시 체인 감사로그(변조 탐지, verify) + JSONL 영속 |
...
- **Audit trail:** ✅ 해시 체인 감사로그(`audit/`) — 파이프라인 이벤트 append-only 기록, `verify()`가
  사후 변조/재정렬 포착(tamper-evident). + approval 기록·소스 hash·결정론 재현.
...
- ✅ **완료:** 룰엔진, Extractor+Citation Assurance(무료 LLM 제공자 지원), **RAG/retrieval(BM25+recall@k)**,
  ...
  임계값, Validation Report(md+HTML+**정식 17쪽 서사**), **Audit Trail(해시 체인)**, Model/System Card, AI Risk Register.
- ◐ **부분:** Temporal Policy Resolver(지역만), Minimal UI(2화면 데이터바인딩).
- ⬜ **계획:** 임베딩 검색 라이브 실행(무료 로컬), 다문서 골드셋 확대, 자동 LLM 실행, 라이브 파이어,
