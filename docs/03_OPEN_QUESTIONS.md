...
## Q10. 非규제(수도권) 유주택 LTV 기준선 — `[🟡 부분 해결 2026-08-18 · 잔여 미결]`

### ✅ 해결된 부분 — 다주택(수도권): **0%**, 원문에 이미 있었다

원문 재검토 결과 **다주택 구간은 이미 확정 사실(C06)로 기록돼 있었고, 엔진이 그 범위를 좁게 구현**하고 있었다.

> "다주택자는 수도권 內 주택구입시 **규제지역 여부와 무관하게** LTV 0% 적용" — FSC p2 / FAQ Q1 ※

"규제지역 여부와 무관"인데 엔진은 이 규칙을 `REGULATED` 분기 **안에서만** 적용했다(P3).
그래서 시행 전(非규제) 다주택이 전부 `OWNER_BASELINE_UNKNOWN`으로 새고 있었다 — 명세 공백이 아니라
**구현 결함**이었다. 시점 무관인 근거도 원문에 있다(C14):

> "금번 조치로 규제지역으로 지정된 지역에 대해서는 **旣 마련된 규정**에 따라 ... 7.1일부터 즉시 적용"

즉 6·30 지정은 새 규칙을 만든 게 아니라 기존 규칙을 발동시킨 것이므로, 수도권 다주택 0%는 6.30
이전에도 성립한다. → `05_RULE_SPEC §E` 에 **P0c**(수도권 다주택 → 0%, 경과규정보다 앞) 신설.

- **효과:** 자동판정 불가 **33.3%(666건) → 22.4%(447건)**. 219건이 "영향 없음(0%→0%)"으로 정상 판정.
- **✍️ 사용자 확정 필요:** 위 P0c 해석(특히 "시행 전에도 0%")이 실무와 일치하는지 도메인 검수.
  `05_RULE_SPEC §C-2 보강` 표에 🤖 초안으로 표시돼 있다.

### ⛔ 잔여 미결 — 비처분 1주택(수도권): **원문에 없음** (397건, 19.9%)

FAQ Q2 표 **주1)** 이 "무주택자(처분조건부 1주택자 포함) 기준"으로 非규제(수도권) 열 전체를 한정한다.
따라서 비처분 1주택 값은 원문 어디에도 없다(C15). MOLIT의 "유주택 60%"는 **수도권 外** 값이라 전용 불가.

**사용자 판단 필요 — 6·30 이전 非규제 수도권에서 비처분 1주택자의 LTV 기준선은?**

| 선택지 | 결과 | 대가 |
|---|---|---|
| ① 감독규정 원문에서 확정값을 찾아 명세에 추가 | 자동판정률 최대치 | 추가 원문 조사 필요(은행업감독규정 등) |
| ② 코어 스코프에서 명시적 제외 | 매트릭스에서 Discovery로 분리, escalation 아님 | "유주택은 다루지 않는다"를 범위로 못박게 됨 |
| ③ 현행 유지(계속 escalate) | 지금 상태 | **자동화율 상한이 구조적으로 약 78%로 고정**됨을 문서화해야 함 |

추정값을 넣는 선택지는 없다 — LOCKED §4 위반.

### 부수 미결 — 비수도권 다주택 (50건, 2.5%)
수도권 0% 규칙은 수도권 限이므로 비수도권 非규제 다주택 값도 없다.
`MULTI_HOME_BASELINE_UNKNOWN`으로 분리 라벨링했다(6·30 시나리오 밖이라 우선순위 낮음).

- 상세: `docs/eval/impact_matrix_6_30.md` "규정 해석 공백 해소" 행, `05_RULE_SPEC §C-2 보강`.
