...
## Q10. 非규제지역 유주택 LTV 기준선 — `[대기, 도메인 확인 필요 · 영향 큼]`
- **발견 경위:** Impact Matrix E2E에서 합성 포트폴리오 2,000건을 시행 전/후로 돌린 결과
  **33.3%(666건)가 자동 판정 불가**로 떨어졌고, 사유가 전부 `OWNER_BASELINE_UNKNOWN` 하나였다.
- **원인:** `05_RULE_SPEC` §C-2가 非규제(수도권) 기준선을 **무주택 70%만** 정의한다. 유주택자의
  비규제 기준선이 없어 시행 **전** 값을 세울 수 없고, 따라서 before/after 비교 자체가 성립하지 않는다.
  엔진은 값을 추정하지 않고 `NEEDS_HUMAN_REVIEW`로 escalate한다(LOCKED §4 준수).
- **왜 중요한가:** 유닛 테스트 30건에서는 "케이스 1건"이었지만 포트폴리오 규모에서는 **1/3**이다.
  E2E를 돌려보기 전에는 이 공백의 크기를 알 수 없었다.
- **사용자 판단 필요:** 6·30 이전 非규제 수도권에서 유주택자(비처분 1주택 / 다주택)의 LTV 기준선은?
  ①원문·기존 감독규정에서 확정값을 찾아 명세에 추가 ②코어 스코프에서 명시적으로 제외
  ③현행 유지(계속 escalate) — 다만 이 경우 자동화율 상한이 구조적으로 67%로 고정됨을 문서화.
- 상세: `docs/eval/impact_matrix_6_30.md` "규정 해석 공백 해소" 행.
