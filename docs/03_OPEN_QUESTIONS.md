...
## Q9. D-02(예외 누락) 대책 선택 — `[✅ 해결 2026-08-18]`

- **채택: ① 문서별 개별 추출 후 union.** Change Completeness 79%→**100%**,
  Exception Recall 50%→**100%**. 호출 3배(무과금이라 지출은 0원), 소요 ~8분.
- **기각: 유사도 기반 병합.** 골드 점수는 통과했으나 실제 병합 내용을 열어 보니 서로 다른
  차주 유형·임계값·규제가 합쳐지고 있었다. 대신 **문서 간 병합만** 허용한다(§6-4).
- **전제 작업:** 골드가 required 4개로 너무 얇아 튜닝 근거가 되지 못했다 → v2로 확장(19+4).
  확장 후 단일 패스의 실제 성적이 79%/50%로 드러났다.
- 상세: `docs/eval/EXTRACTOR_RUN_REPORT.md` §6.
...
- **✅ 도메인 검수 완료(2026-08-18, 사용자):** "시행 전에도 0% 맞다" — P0c 확정.
  `05_RULE_SPEC §C-2 보강` 표 ✅ 전환, `regulatory_facts` C06·C14 ✅확정.
