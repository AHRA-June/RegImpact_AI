...
- **마지막 갱신:** 2026-08-23 (**현업용 상품설명서 신설** — 산출물에서 생성되는 B2B 설명서 + 낡은 테스트 수 주장 3곳 정정)
- **개발 브랜치:** `claude/product-description-field-focused-x1x4in` (기본 브랜치 = `claude/portfolio-project-planning-9sip11`)
- **전체 단계:** 🟢 **Phase 2 마무리 + 공모전 트랙** — 배포 완료, 남은 것은 QA 골드 검수와 Phase 3 진입
- **테스트:** 668개 통과(브라우저 검사 10건 포함 — 로컬 기본 실행에서는 skip, CI 에서 전량 실행)
  · **비용: 0원** (유료 API 키 미사용)
...
✅ **현업용 상품설명서 (2026-08-23)** — 지금까지 설명서는 고객 면(내 한도 시그널) 하나뿐이었고,
   현업 산출물은 그 문서의 한 절로만 있었다. 현업 면만 다루는 설명서를 신설했다:
   `src/regimpact/business/ops_description.py` 가 `ValidationEvidence` 만 읽어 조립하고,
   `examples/build_ops_description.py` 가 `docs/business/OPS_PRODUCT_DESCRIPTION.md` 로 쓰며,
   사이트에는 `service_ops.html` 로 뜬다(메뉴 "제안 문서"). 고객 설명서는 사람이 쓴 문서를
   감싸지만 이쪽은 **수치가 전부 실행 산출물이라 생성한다** — 테스트가 커밋된 문서와 지금
   시스템의 렌더 결과를 대조하므로, 코드가 바뀌었는데 문서를 안 돌리면 실패한다.
   곁가지로 **테스트 수 주장이 낡은 채 숨어 있던 3곳을 잡았다**(굵게 쓴 `**590개**`와 지표
   타일 `569` 가 정규식 한 형태만 보던 검사를 빠져나가고 있었다 — 검사도 함께 조였다).
...
✅ **지원서 최종 점검 (2026-08-21)** — 제출물을 전부 실행해 대조했더니 **네 곳이 이미
   실제와 갈라져 있었다**: 한 줄 설명 글자 수 3개 전부(66/39/86 → 74/47/77), 덱의 테스트
   수(590), **덱의 목표 역산 처방 3줄**(엔진이 불가로 판정하는 처방을 가능하다고 적었고
   "만기 40년"은 규제지역 상한 30년과 어긋났다 — 심사자가 데모에 같은 값을 넣으면 다른
   화면을 본다), 데모 영상(6단계 '지켜보기' 이전 화면). 공통 원인은 **제출물 생성기가
   저장소 밖에 있어 아무도 대조하지 못한 것**이라, 값을 다시 적는 대신 출처를 옮겼다 —
   `tools/deck_facts.py`(엔진에서 계산·화면과 같은 반올림)·`tools/build_deck.js`·
   `tools/record_demo.py`(시간표를 녹화하며 잰다) 편입 + 검사 4건.
   상세: `02_DECISION_LOG.md` 2026-08-21.
