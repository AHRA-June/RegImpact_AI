...
- **마지막 갱신:** 2026-08-19 (세션 종료 — PR #20~#24 전부 병합·배포 완료)
- **개발 브랜치:** `claude/remaining-work-zwayt7` (기본 브랜치와 동기화됨 — 문서 커밋만 앞섬)
- **전체 단계:** 🟢 **Phase 2 마무리** — 배포 완료 + 그래프·검색·코퍼스 확장까지. 남은 것은 아래 "내일 할 일"
- **테스트:** 521개 통과(1 skipped) · **비용: 0원** (유료 API 키 미사용)
...
## 내일 할 일 (2026-08-20 — 우선순위 순)

### A. 📄 문서 입고 대기 → 즉시 실행 경로 (사용자가 2020~2025 규제 문서 수집 중)
- **해제 보도자료(2022~2023 국토부)가 오면**: `tools/ingest_source.py` 봉인·추출 →
  2020 구간 닫기(effective_to) → `MOLIT_20200617` confirm() → 신설 코드 21개 버전 구간 등록 →
  SEJONG·CHEONGJU 비규제 구간 정정 → **시점 질의 골드 초안** 작성(🤖) →
  미측정 지표(시점·정책 버전 일관성) 가동. 코퍼스 recall 재실측(시점 필터 대책 4개로 확대).
- 2016·2017 지정 원문이 오면: 기존 확정 정책 2건에 원문 근거 연결(2025 때와 같은 절차).
- 그 사이 추가 지정·부분 해제 문서: 타임라인 보강.

### B. ✍️ 플레이그라운드 단독 웹앱 분리 — 사용자 고민 중 (지원사업용)
- 검토 결과는 세션 대화(2026-08-19 마지막)에 있음. 요지: **가능하고 공수 작음** —
  이미 자기완결 정적 HTML + JS 엔진(전 케이스 대조)이라 서버 불필요. 권고는 코드베이스 분리가
  아니라 **단독 빌드 타깃**(전용 URL·브랜딩·면책 고지 + PWA manifest) — 검증 하네스·업데이트
  파이프라인을 그대로 공유. 결정 필요: ①B2C 포지셔닝(사업계획 v1 정합) ②판정 범위(현재 LTV만 —
  한도·DSR 확장 여부) ③면책 문구. 결정되면 시제품 빌드 타깃은 반나절~1일.

### C. ✍️ QA 골드 DEV 검수 계속 — 4/40 → 잔여 36
- 검수표: `docs/eval/QA_GOLD_REVIEW.md` (`python tools/build_qa_review.py`)
- 다음 볼 것: 베이스라인이 놓친 앵커 2문항(EFF-001·EFF-002), 그다음 카테고리 순서대로.
- 확정되면 `tools/gold_dev.py`에 `authored_by="human_confirmed"` 지정 후 재생성.

### D. ✍️ metrics_spec 임계값 확정 — Phase 3 의 전제
...
### E. Phase 3 — LOCKED / CHALLENGE 최초 실행 (C·D 완료 후)
...
| 항목 | 위치 | 영향 |
|---|---|---|
| 2020~2025 규제 문서 수집 (특히 해제 보도자료) | 사용자 수집 중 | 2020 정책 확정 + 시점 지표 가동의 열쇠 |
| 플레이그라운드 단독 웹앱 분리 여부 | 내일 할 일 §B | 지원사업 데모 형태 |
| QA 골드 DEV 잔여 36문항 검수 | `docs/eval/QA_GOLD_REVIEW.md` | QA 지표의 절대값 신뢰도 |
...
- **2026-08-19(세션 종료)** — 📦 **인계 정리.** 이 세션의 산출: PR #20~#24 전부 병합·배포
  (UX 정비 + 문서 등록 화면 / 검증 요약 1페이지 / 그래프 + 검색 + 페이지 설명 / 코퍼스 확장 +
  시점 필터 / 2020 지역 이관 확정). QA 검수 4/40, 2020 정책 DRAFT(해제 원문 대기). 테스트 521.
  상태판을 "내일 할 일 A~E" 체계로 재구성 — **A(문서 입고 경로)가 최우선**, B(플레이그라운드
  웹앱 분리)는 사용자 고민 중이라 검토 요지를 §B에 남김. 브랜치는 기본과 동기화(문서 커밋만 앞섬).
