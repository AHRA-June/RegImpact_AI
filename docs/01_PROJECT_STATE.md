...
## ✅ 방금 완료 (2026-08-18) — Policy Version DB + 정책 업로드 (Temporal Policy Resolver)
사용자 요청: "새 정책이 나왔을 때 업로드하는 기능 + 현재/다음 정책을 날짜로 버전 관리."
브리프 §7·§19가 스키마를 이미 확정해 둔 항목이고 §18 "자르면 안 되는 것"에 들어 있다.

- **`src/regimpact/policy/`** 신설 (version · registry · resolver · ingest)
- **저장소는 git 안의 JSON** `docs/policies/*.json` — 정책 버전은 감사 대상이고 git 이력이 곧
  "누가 언제 무엇을 확정했는가"의 기록이다(LOCKED §10).
- **시드 4건:** MOLIT_20161103(강남4구 조정) → MOLIT_20170803(투기과열) → MOLIT_20251016(서울21+경기12)
  → FSC_20260630(경기3). 전부 MOLIT 참고2 현황표에서 유도. **정책 delta 합집합 == 지역 기준선의
  규제지역 집합**을 테스트가 검사(두 구조 드리프트 탐지).
- **Temporal Policy Resolver:** `current_policy` / `upcoming`(다음 정책) / `previous_policy`(시행일 직전
  유효 정책, supersedes 체인) / `timeline`(지난·현재·예정·초안) / `detect_overlaps`(우선순위 검토 표시).
- **DRAFT 는 판정에 쓰이지 않는다(LOCKED §4).** 업로드·LLM 추출은 항상 DRAFT+AI_DRAFT, 사람이
  `confirm()` 해야 CONFIRMED+HUMAN_CONFIRMED. **매핑 못 한 지역명은 조용히 버리지 않고 보고**한다.
- **오버레이 방식:** 기준선을 바꾸지 않는 순수 함수(`resolve_region_with_policies`). delta 가 없으면
  기준선과 완전히 동일 → 기존 판정 무영향(테스트 고정).
- **업로드 UI:** Streamlit "정책 버전" 탭 — 원문 업로드 → SHA-256 → 메타·지역 입력 → 초안 →
  적용 미리보기 + 중첩 경고 → 사람 확정 → 세션 등록 + JSON 다운로드
  (Cloud 파일시스템은 휘발이라 "저장소에 커밋해야 영구 반영"을 화면에 명시).
- **E2E 반영:** 파이프라인 2단계가 Policy Version DB 를 읽어 현재/직전/예정을 보고 · 검증보고서에
  **정책 버전 타임라인** 섹션 추가.
- 테스트 35건 추가(총 146).

## ✅ 같은 날 완료 (2026-08-18) — Q12 해소: 입력 모순 3건 추가
- 채택: 처분조건부인데 보유 0채 / 처분조건부인데 다주택 / house_count 음수 (§A 스키마 근거).
- **미채택(정정):** 계약금만 있고 계약 없음 · 계약금이 계약보다 빠름 — **가계약금**이 실무에 실재하므로
  두 후보는 같은 질문이다. 기계적으로 막으면 정상 건을 사람 검토로 보낸다. §F G2 가 쌍을 요구하므로
  경과규정이 성립하지 않을 뿐이고 그건 이미 올바른 동작. 회귀 `CFL-16` 으로 고정.
- 회귀 60 케이스 100% (CONFLICT 16).

## ✅ 같은 날 완료 (2026-08-18) — Q8 해소: 입력 무결성 게이트(P0c)
...
- **회귀 60 케이스 100%** (EXCEPTION 10 / GRANDFATHERING 9 / CONFLICT 8), pytest 80.
...
- **2026-08-18** — ✅ **Q9·Q10 사용자 확정 반영 + 경과규정 버그(Q11) 수정.** 비규제 유주택 60%(비수도권 한정, 수도권은 근거 부재로 escalation 유지), 다주택 판정을 지역 분기 앞으로 이동(수도권이면 규제 무관 0%, 비수도권 비규제는 유주택 기준 60%). 반영 중 경과규정의 '종전규정'을 70%로 하드코딩한 버그 발견 — 이미 규제지역이던 강남에 경과규정이 붙으면 70%가 나왔다 → 컷오프 시점 재판정으로 정정. `05_RULE_SPEC` §C-2·§F·§H 개정. 오라클을 선언적 규칙표+범용 해석기로 재작성(구조적 독립 실현). 회귀 60 케이스 100%, pytest 80, JS 52/52 + 지역 241×7 일치.
