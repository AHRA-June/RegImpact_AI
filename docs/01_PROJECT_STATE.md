...
1. **지역 레지스트리 결함** — 강남·서초·송파·용산이 `NON_REGULATED` 로 판정된다.
   `REGION_VERSIONS` 에 6·30 신규 3곳만 있고 기존 규제지역이 통째로 빠져 있다.
   → 무주택 차주가 강남에서 LTV 70%를 받는다. 상세·선택지는 `docs/07_BRANCH_TRIAGE.md` §1.
2. **브랜치 이식** — 원격 브랜치 11개 분류 완료. 살릴 기능 22건 목록: `docs/07_BRANCH_TRIAGE.md` §3.
   ✍️ `kus0w7`·`71qh9m`·`o2geks` 3개는 이식할 것이 없어 삭제 승인만 받으면 된다.
