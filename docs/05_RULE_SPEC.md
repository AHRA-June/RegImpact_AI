...
| `max_ltv` | float | 적용 최대 LTV (0.0~1.0) — **코어 1차 출력** |
| `max_dti` | float? | 적용 DTI (아파트 限). 🔺regulated_type별(투기과열 0.40/조정 0.50). ✍️Q: 코어 포함? |
| `max_loan_amount` | int? | 주택가격 구간별 한도(15억↓6억/15~25억4억/25억↑2억). ✍️Q: 코어 포함? |
...
> ✍️ **Q-스코프:** DTI·최대한도를 **코어 엔진 출력에 포함**할까요, 아니면 LTV만 코어로 두고 DTI/한도는 참고표로 둘까요?
