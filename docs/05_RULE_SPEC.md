...
#### C-1/C-2 재확인 (2026-08-18, 골드 검수) ✅

골드셋 작성 중 원문 **텍스트 추출본**에서 유도한 값이 이 표와 충돌해 재확인했고, **표가 맞음**을 확정했다.

| 쟁점 | 텍스트 추출본이 시사한 값 | ✅ 확정 |
|---|---|---|
| 非규제(수도권) 기준선 LTV | 60% | **70%** (C-2 B1) |
| 규제지역 일반 LTV | 조정 50% / 투기과열 40% | **둘 다 40%** (C-1 R1). 40/50은 **DTI** |
| 서민·실수요자 | "좌동(변화 없음)" | **70% → 60% 변경 있음** (C-2 B1 → C-1 R3) |

**원인:** FAQ 원문이 HWP다. 텍스트로 추출하면 Q2 표의 **LTV 열과 DTI 열이 한 줄로 뭉개져**
40/50/60이 전부 LTV처럼 읽힌다. 이 표의 값은 사용자가 **원본 이미지로 확정한 것**이므로 권위는 이쪽에 있다.

> ⚠️ **원문 텍스트만 보고 이 구간의 값을 유도하지 말 것.** 반드시 이 표를 기준으로 한다.
> 골드가 이 표와 어긋나면 `regimpact.eval.check_gold_against_spec()`이 자동으로 잡는다.

#### C-2 보강 (2026-08-18) — 유주택 구간 정리 ✅ **확정 (2026-08-18, 사용자)**
...
| **다주택 (수도권)** | **0.00** | FSC p2 · FAQ Q1 ※ (아래 원문) | ✅ **확정** (시행 전 포함) |
...
> ⚠️ **비처분 1주택 기준값은 원문에 없다(C15).** MOLIT의 유주택 60%는 수도권 外 값이므로 전용 금지.
> 값을 추정하면 LOCKED §4 위반 → 엔진은 `OWNER_BASELINE_UNKNOWN`으로 escalate 한다.
>
> ✅ **스코프 확정(2026-08-18, 사용자):** 그렇다고 **유주택자를 코어 스코프에서 빼지 않는다.**
> 실제 고객 모집단에 유주택자가 존재하므로, 범위에서 제외해 지표를 좋아 보이게 하는 것은 회피다.
> 유주택자는 판정 대상이며, 기준값이 없는 구간만 escalation으로 남긴다.
>
> 참고로 **규제지역 유주택은 시행일 LTV가 0%로 확정(R5)** 되므로 심사 자체는 자동 판정된다.
> 미상인 것은 시행 전 기준값이 없어 생기는 **변화량 비교**뿐이다 — 이 둘을 구분해 집계한다
> (`decision_coverage` 91.4% vs `impact_coverage` 74.1%).
...
def evaluate_mortgage_ltv(inp) -> Result:
    ...
    # P0c. 수도권 다주택 → 0% (규제 여부·시점·경과규정 무관) — C-2 B3, ✅2026-08-18 확정
