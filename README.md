...
| `src/regimpact/rule_catalog.py` | **내규 영향도 맵** — 규제 변경→모의 여신 내규 대장 매핑(수정필요/검토/간접/무관), 과잉플래그 없음 증명, 승인 PENDING |
| `src/regimpact/stress.py` | **스트레스 테스트** — 극단 시나리오 배터리·역스트레스(결론 뒤집힘 경계)·차등 퍼징·대규모 부하 |
...
| `examples/` | 6·30 룰엔진 데모 / Extractor 실행 / TC 회귀 데모 / Impact Matrix 데모 / 추출→임팩트 E2E / HTML 리포트 생성 / 골드셋 포트폴리오 / 룰 변경안 / 가중 모집단 / 여력 영향 금액 / 민감도 분석 / 내규 영향도 / 스트레스 테스트 |
