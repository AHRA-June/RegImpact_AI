...
| `tests/` | 테스트 하네스 (pytest, 105개) |
| `examples/` | 6·30 룰엔진 데모 / Extractor 실행 / TC 회귀 데모 / Impact Matrix 데모 / 추출→임팩트 E2E / HTML 리포트 생성 / 골드셋 포트폴리오 / 룰 변경안 |
...
🟢 **Phase 1~2 진행** — 룰엔진 v1 + Extractor + TC Generator/Rule-Regression + Impact Matrix E2E + HTML 리포트(6·30 관통) 구현(테스트 105개 통과). 자세한 내용은 `docs/01_PROJECT_STATE.md` 참고.

```bash
python -m pytest && python examples/demo_6_30.py && python examples/demo_tc_regression.py && python examples/demo_impact_matrix.py && python examples/demo_portfolio.py && python examples/gen_report.py
```
